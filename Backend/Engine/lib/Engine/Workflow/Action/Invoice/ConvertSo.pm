package Engine::Workflow::Action::Invoice::ConvertSo;
use strict;
...
use Engine::Model::Invoice::InvoiceHead;
use Engine::Model::Invoice::InvoiceItem;
...
sub execute ($self, $wf) {
    ...
    if($context->param('salesorders_pkey') > 0) {
        ...
        my $order = Engine::Helpers::Salesorder::Salesorders->new(
            pg => $pg
        ...
        eval {
            ...
            $invoice_pkey = Engine::Model::Invoice::InvoiceHead->new(
                db => $db
            )->insert(
            ...
            foreach my $item (@{$order->{items}}) {
                ...
                Engine::Model::Invoice::InvoiceItem->new(
                    db => $db
                )->insert(
